...
namespace Utility.IOs
{
    ...
    public static class DAMCommand
    {
        ...
        {
            if (closing)
                sp.WriteData(SerialPortUtil.HexToByte("FE 0F 00 00 00 04 01 FF 31 D2"));
            else
                sp.WriteData(SerialPortUtil.HexToByte("FE 0F 00 00 00 04 01 00 71 92"));
        }
        ...
        public static void CheckInput(this SerialPortUtil sp, int start, int num, System.Windows.Forms.Timer timer)
        {
            timer.Tick += (sender, e) => {
                sp.WriteData(CheckInputData(start, num));
            };
        }
